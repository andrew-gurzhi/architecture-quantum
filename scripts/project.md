...
у нас всего 5–20 тысяч чанков — FlatL2 даёт 100% точный поиск и летает
...
Подготавливает промпт для LLM (в нашем случае — через OpenRouter + мой API-ключ OpenAI-совместимой модели(ключ с небольшим лимитом и ограничен месяцем)).
